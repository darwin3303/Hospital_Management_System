...
class PharmacyService:
    ...
    def dispense(self, payload: DispenseRequest, actor: User) -> PharmacyDispense:
        ...
        if domain.is_expired(medicine.expiry_date):
            raise ValidationAppError("This medicine has expired and cannot be dispensed.",
                                      code="MEDICINE_EXPIRED")

        if not domain.has_sufficient_stock(medicine.quantity_in_stock, payload.quantity):
            raise ValidationAppError("Insufficient stock to fulfill this quantity.",
                                      code="INSUFFICIENT_STOCK",
                                      details={"available": medicine.quantity_in_stock})
